"""
Write text onto an ExtendedSurface object.
...
class TextSurface():
	...
	def __init__(self, calling_surface):
		...
		Keyword arguments:
			calling_surface (ExtendedSurface) -- the surface onto which
		...
		Class attributes:
			...
			exterior_extended_surface (ExtendedSurface) -- the Surface
			...
			interior_extended_surface (ExtendedSurface) -- the Surface
	...
	def write(self, text, x, y, font, **kwargs):
		...
		from .ExtendedSurface import ExtendedSurface
		...
		if self.font_size < 0:
			logging.error(f"Cannot write text given these constraints: "
		...
		self.text_width, self.text_height, self.text_lines_metadata =\
			self._get_text_dimensions(self.text_lines)

		# Create the interior ExtendedSurface to hold the text block
		self.interior_extended_surface = ExtendedSurface(
			int(self.text_width + self.outline),
			int(self.text_height + self.outline)
			)

		# Write the text to the interior
		self._write_text_to_interior()

		# Create the exterior ExtendedSurface to hold the text block within
		# the given padding
		self.exterior_extended_surface = ExtendedSurface(
	...
	def _calculate_max_font_size(self, text):
		...
		assert text_width <= self.max_width and text_height <= self.max_height, \
	...
	def _create_font_face(self, face_index=0, load_options=0):
		...
		try:
			...
			if status != ft_err_ok:
				raise RuntimeError(
					f"Error {status} creating FreeType font face for {self.font}")
			...
			if _cairo_so.cairo_font_face_get_user_data(
					cr_face, ct.byref(_ft_destroy_key)) is None:
	...
	def _get_text_dimensions(self, lines, font_size=None):
		...
		font_ascent, font_descent =\
			self.calling_surface.context.font_extents()[0:2]
	...
	def _split_text_into_lines(self, text, font_size=None):
		...
		for word in words:
			...
			line_width = self._get_text_dimensions(
				[' '.join(line + [split_words[0]])], font_size)[0]
	...
	def _write_text_to_interior(self):
		"""
		Write the lines of text to the interior ExtendedSurface.
		...
		for index, line in enumerate(self.text_lines):
			...
			if self.alignment == "left":
				...
				space_width = (
					((line_width - no_spaces_width))
					/ max((len(line.split()) - 1), 1)
					)
